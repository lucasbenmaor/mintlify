...
<ParamField body="product_dimensions" type="object">
  ...
  <Warning>
    Send both objects, each with a valid `size`. They are optional in the contract, but we work out
    the shipping weight from the two together, and today a product that arrives without them does not
    get created — and no error comes back to say so. See
    [Checks that run after you get the response](#checks-that-run-after-you-get-the-response).
  </Warning>
</ParamField>
...
<ParamField body="images" type="array">
  ...
  The response returns this field in a different shape: a flat array of URL strings, not the objects
  you sent.
</ParamField>
...
- `package_dimensions.weight` missing, unless the product carries an ASIN and we manage to recover
  the weight from Amazon — the `size` is never recovered that way
