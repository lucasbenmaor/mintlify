...
<ParamField body="product_dimensions" type="object">
  ...
  <Note>
    Both objects are optional because we complete them from the Amazon listing matched by the
...
- no shipping weight can be worked out: `package_dimensions.size`, `product_dimensions.size` or
  `package_dimensions.weight` missing from your request and not recoverable from the Amazon match
  either
